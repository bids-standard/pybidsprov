...
TEST_DATASET_0 = BIDSLayout(join(TEST_DATA_DIR, 'provenance_ds00'), validate=False, is_derivative=True)
TEST_DATASET_1 = BIDSLayout(join(TEST_DATA_DIR, 'provenance_ds01'), validate=False, is_derivative=True)
TEST_DATAFILE_1 = TEST_DATASET_1.get()[21]
TEST_DESCRIPTIONFILE_1 = TEST_DATASET_1.get()[0]
TEST_DATASET_2 = BIDSLayout(join(TEST_DATA_DIR, 'provenance_ds02'), validate=False, is_derivative=False)
...
class TestMergeFunctions(unittest.TestCase):
    ...
    def test_filter_provenance_group(self):
        ...
        assert out_list[1].filename == 'prov-spm_io.json'
    ...
    def test_get_provenance_files(self):
        ...
        assert not get_provenance_files(TEST_DATASET_0, 'io')
        ...
        assert not get_provenance_files(TEST_DATASET_1, 'io', 'dcm2niix')
        out_list = get_provenance_files(TEST_DATASET_1, 'io')
        assert len(out_list) == 2
        assert out_list[0].relpath == join('prov', 'prov-preprocessing_io.json')
        assert out_list[0].filename == 'prov-preprocessing_io.json'
        assert out_list[1].relpath == join('prov', 'prov-spm_io.json')
        assert out_list[1].filename == 'prov-spm_io.json'
        out_list = get_provenance_files(TEST_DATASET_1, 'io', 'spm')
        assert len(out_list) == 1
        assert out_list[0].filename == 'prov-spm_io.json'
            ...
            'Checksum': [
                {
                    ...
                    'ChecksumValue': 'f29cb68cce4cb3aa2ccbc791aceff3705a23e07dfc40c045a7ce3879ebc1f338'
                }
            ]
        }
